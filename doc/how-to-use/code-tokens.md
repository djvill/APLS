...
nav_order: 65
last_modified_date: 2025-11-03T15:37:54-05:00
---
...
On this page, we'll discuss three common coding scenarios: variables for which you need to [listen to transcript audio](#coding-tokens-by-listening-to-audio) in order to code them, variables that can be coded based on the [text of the transcript](#coding-tokens-by-reading-context) alone, and variables for which codes can be [extracted from annotations](#coding-tokens-by-extracting-annotations) without you needing to manually code tokens.
While we'll focus on coding variables, these scenarios can also apply to [predictors](#coding-predictors).
...
In some cases, you still might need to categorize exported annotations into variants in order to get your codes.
...
It sets up an interface that makes coding faster and easier by playing tokens one-at-a-time, giving you buttons to press for coding, and writing your codes to a CSV file.
...
1. Download Praat from <https://praat.org/> and install it on your computer.
   - If you already have Praat, make sure it's at least version 6.4.32.
1. Download `CodeTokens.praat`
   ...
   - When you open `CodeTokens.praat`, it should automatically open in Praat if you've already downloaded Praat.
...
This will bring up the basic settings window.
...
The coding screen consists of two windows:
...
- Coding predictors by [extracting annotations](#coding-tokens-by-extracting-annotations). Works well when the predictor can come:
  ...
  - From relabeled annotations (like providing nicer labels for <span class="layer">part_of_speech</span> tags)
