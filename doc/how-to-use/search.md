...
The screengrab below has all projects toggled on to show all the layers available for searching.
...
Selecting a layer in the layer selection menu will cause a pattern input field appear for that layer below the layer selection menu.

{:.note}
> Layers that are selected with the layer selection menu are only applied to the search if something is entered into their pattern input fields.

### Pattern input fields

The pattern input fields are where you specify the pattern you'd like to search for.

{:.note}
> Layered search is by word, so the patterns you search will be matched against individual words instead of entire utterances. 
...
At the top-left of the input field section, you'll see the name of the layer that input field will apply to.
...
These layers allow you to "anchor" your search pattern to match the beginning or the end of the matching word using the lock icons.

The following layers have pattern input fields that work similarly to the <span class="layer">orthography</span> layer, with the addition of anchoring options.
...
An example of a more complex search, finding all verbs that begin with a consonant which follow the word "the", is explained in the `TRY IT!` below.
