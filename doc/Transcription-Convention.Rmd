...
### Unfinished words and novel words

<!-- ADD THE FOLLOWING SOMEWHERE:
			There are 3 situations in which you need to use a "pronounce code":
			- hesitations/incomplete words
			- idiosyncratic pronunciations
			- valid words that shouldn't be in APLS's custom dictionary

			- Add syllabification & stress to pronounce codes
-->
...
- Words that are unfinished are represented in a special way: with a tilde to mark incompleteness and at least one code to help LaBB-CAT assign phonemes to the unfinished word. 
	...
	- Then, attach a "pronounce code" with **no space after the tilde**. The pronounce code uses the [DISC phonemic alphabet](Phonemic-Transcription) to give a phonological representation of what was said, in square brackets (`[hEz@]`, where DISC `E` & `@` = IPA ε & ə). Note that DISC representations are **phonological, not phonetic**
